...
router.post("/report/xlsx", async (req, res) => {
    try {
        ...
        res.status(500).json({ message: "Error generating reports : " + error.message });
    }
});
        ...
        if (req) {
            ...
            if (dog.vetDetails) {
                ...
                Object.keys(dog.vetDetails._doc).map((key, i) => {
                    if (!['surgeryPhoto', 'additionalPhotos', 'surgeryDate', 'createdAt', 'updatedAt', 'additionalNotesPhotos'].includes(key)) {
            ...
            if (dog.careTakerDetails) {
                ...
                const ctws = XLSX.utils.json_to_sheet([...careTakerDetails, ...reportsDetails]);
            ...
            res.setHeader('Content-Disposition', `attachment; filename=${dog.dogName} (${dog._id.toString()}).xlsx`);
